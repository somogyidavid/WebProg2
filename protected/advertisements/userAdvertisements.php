<?php
    ...
    $query = "SELECT ad.id, ad.title, ad.userId, det.licencePlate, brands.brand_name, models.model_name, det.vintage, det.price, det.image FROM advertisements ad INNER JOIN advertisementdetails det ON ad.id = det.advertisementId INNER JOIN brands ON det.brand = brands.id INNER JOIN models ON det.model = models.id WHERE ad.userId=:id";
    $params = [
        ':id' => $_SESSION['uid']
    ];
